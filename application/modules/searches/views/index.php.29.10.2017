			...
			<div class="dataTable_wrapper">
				<table class="table table-striped table-bordered table-hover" id="dataTables-example">
						...
						<tr>
	                    	...
	                    	<th><?=lang('tickets:creator')?></th>
	                    	<th><?=lang('tickets:created_group')?></th>
	                    	<th><?=lang('tickets:created_on')?></th>
	                    	<th><?=lang('tickets:category')?></th>
	                    	<th><?=lang('tickets:status')?></th>
	                    	<th><?=lang('tickets:area_code')?></th>
	            	...
	            	<tbody>
						...
						<tr class="odd gradeX">
							...
							<td><?=htmlspecialchars($search_ticket->created_user,ENT_QUOTES,'UTF-8')?></td>
							<td><?=htmlspecialchars($search_ticket->created_group,ENT_QUOTES,'UTF-8')?></td>
							<td><?=htmlspecialchars(date($this->config->item('log_date_format'), $search_ticket->created_at),ENT_QUOTES,'UTF-8')?></td>
							...
							<td><?=htmlspecialchars($search_ticket->name,ENT_QUOTES,'UTF-8')?></td>
							<td><?=htmlspecialchars($search_ticket->product_phone,ENT_QUOTES,'UTF-8')?></td>
							<td><?php //echo ( ! empty($edit_ticket) ? anchor("tickets/edit/".$search_ticket->id, 'Edit') : '');
								echo anchor("tickets/edit/".$search_ticket->id, 'Edit');?></td>
							<td><?php //echo ( ! empty($view_ticket) ? anchor("tickets/view/".$search_ticket->id, 'View') : '');
								echo anchor("tickets/view/".$search_ticket->id, 'View');?></td>
						</tr>
					<?php endforeach; ?> 
	                </tbody>
	            </table>
