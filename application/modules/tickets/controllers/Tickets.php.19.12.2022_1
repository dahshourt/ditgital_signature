...
//die('fff');
                $custom_fields_data['get_access_fields'] = $get_access_fields;
                $custom_fields_data['group_id'] = $creator_group_id;
                $custom_fields_data['category_id'] = $this->input->post('category_id');//creator_group_id
                $custom_fields_data['closed_by_user']  = $this->input->post('category_id');
                $custom_fields_data['sub_category_id'] = $this->input->post('sub_category_id');
                $custom_fields_data['transfer_group_input'] = array(
                    'current_group' => $creator_group_id,
                    'previous_group' => null,
                ...
                //print_r($this->form_validation->set_value('sub_category_id')); die;
    ...
    public function edit($ticket_id = '') {
        ...
        $tickets_data['transfer_status_input'] = 'current_group=' . $tickets_data['group_id'] . ' and transfer_group=' . $tickets_data['group_id'] . ' and previous_group=' . $tickets_data['previous_group_id'] . ' and problem=' . $tickets_data['sub_category_id'] . ' and current_status IS NULL';
...
/////////////////////view_ticket
    ...
    public function view($ticket_id = '') {
        ...
        $input = array('form_type' => '1', 'category_id' => $tickets_data['category_id'],
            'sub_category_id' => $tickets_data['sub_category_id'], 'group_id' => $tickets_data['creator_group_id']);
        $this->load->module('custom_fields/custom_fields');
        $get_access_fields = $this->custom_fields->get_access_fields1($input);
//		if($tickets_data['status_id'] !=1)
//		{
//			unset($get_access_fields['closed_by_user']); 
//		}
        ...
        $tickets_data['transfer_group_input'] = '"current_group"=' . $tickets_data['group_id'] . ' and "previous_group"=' . $tickets_data['previous_group_id'] . '
                                                and ("current_status" =' . $tickets_data['status_id'] . ' or "current_status" IS NULL)';
        $tickets_data['transfer_status_input'] = 'current_group=' . $tickets_data['group_id'] . ' and transfer_group=' . $tickets_data['group_id'] . ' and previous_group=' . $tickets_data['group_id'] . ' and current_status IS NULL';
        ...
        $data['tickets_data'] = $tickets_data;
        $data['files'] = $this->ticket_model->get_ticket_file_data($ticket_id);
		$data['username']=$this->ticket_model->get_user_name($tickets_data['closed_by_user']);
        $data['ticket_id'] = $ticket_id;
        $data['ticket_logs'] = $this->ticket_model->get_ticket_logs($ticket_id);
