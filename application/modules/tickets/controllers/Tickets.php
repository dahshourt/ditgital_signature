...
class Tickets extends MY_Controller {
	...
	public function create()
    {
		...
		if ($this->input->post('creator_group_id'))
		{
			...
			unset($get_access_fields['closed_by_user']);
	...
	public function ticket_assigned_group()
	{
		if ($this->input->server('REQUEST_METHOD') === 'POST') {
			...
			$this->form_validation->set_rules('manager_id', 'User', 'required');
			$this->form_validation->set_rules('ids[]', 'ticket', 'required');
			if($this->form_validation->run() == FALSE)
			{
				
				$this->session->set_flashdata('error', validation_errors());
				return redirect(site_url('/tickets/ticket_assigned_group'), 'refresh');
				//redirect('/tickets/ticket_assigned_group');
			}
			else
			{
				foreach($this->input->post('ids') as $key=>$ticket_id)
				{
				...
				$this->session->set_flashdata('message', "Ticket(s) assigned successfully");
				return redirect(site_url('/tickets/ticket_assigned_group'), 'refresh');
		...
		$data['users'] = $this->group_model->get_users_per_group($group_id);
		$data['js'] = array('bower_components/datatables/media/js/jquery.dataTables.min.js', 'bower_components/datatables-plugins/integration/bootstrap/3/dataTables.bootstrap.min.js');
		$data['css'] = array('bower_components/datatables-plugins/integration/bootstrap/3/dataTables.bootstrap.css', 'bower_components/datatables-responsive/css/dataTables.responsive.css');
			...
			if ($value_access_field['type'] == 'dropdown')
			{
				switch ($value_access_field['name']) {
					...
					case 'customer_type_id':
                        ...
                        break;
				case 'created_by':
							$data[$value_access_field['name']] =  $this->ticket_model->get_user_by_id($input_fun['created_by']);
							break;
					case 'closed_by_user':
							$data[$value_access_field['name']] =  $this->ticket_model->get_user_by_id($input_fun['closed_by_user']);
							break;
					   default:			
						break;
				}
	...
	public function edit($ticket_id = '')
	{
		...
		{
            // Start insert
            if (($this->input->post('status_id') OR $this->input->post('comment') OR $this->input->post('group_id') OR $this->input->post('manager_id') )&& $this->input->post('submit') == 'Edit Ticket')
            {
                $created_at = time();
                $created_by = $this->user_id;
                ...
                if ($status_id_post && ($status_id_post != $tickets_data['status_id']))
                {
					...
					if($status_id_post == 1) $data_tickets['closed_by_user'] = $this->user_id;
                }
		...
		$data['comment_logs'] = array('type' => 'button', 'content' => 'Comment Logs', 'name' => 'comment_log', 'class' => 'btn btn-default load-comments','value' => 'Comment Logs');
		...
		$data['js'] = array('tickets/js/create.js','tickets/js/ticket_comments.js','bower_components/moment/min/moment.min.js', 'bower_components/eonasdan-bootstrap-datetimepicker/build/js/bootstrap-datetimepicker.min.js',
			'bower_components/datatables/media/js/jquery.dataTables.min.js', 'bower_components/datatables-plugins/integration/bootstrap/3/dataTables.bootstrap.min.js', 'reports/js/calender_from_to.js');
			$data['css'] = array('bower_components/eonasdan-bootstrap-datetimepicker/build/css/bootstrap-datetimepicker.min.css',
			'bower_components/datatables-plugins/integration/bootstrap/3/dataTables.bootstrap.css', 'bower_components/datatables-responsive/css/dataTables.responsive.css');
  ...
  public function view($ticket_id = '')
    {
	    ...
	    $data['ticket_logs']          = $this->ticket_model->get_ticket_logs($ticket_id);
	    $data['comment_logs'] = array('type' => 'button', 'content' => 'Comment Logs', 'name' => 'comment_log', 'class' => 'btn btn-default load-comments','value' => 'Comment Logs');
		$data['js'] = array('tickets/js/ticket_comments.js');
    ...
    // mahmoud bastawisy updates 10-11-2022
	public function ajax_get_comment_logs()
    {
        ...
        $comments_res = $this->ticket_model->get_ticket_comments($ticket_id);
		$data['comments_res'] = $comments_res;
		$response = $this->load->view('tickets/load_comments',$data,TRUE);
		echo $response;
