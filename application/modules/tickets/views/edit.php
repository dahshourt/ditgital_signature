	...
	<div class="panel-body">
			<div class="dataTable_wrapper">
				<table class="table table-striped table-bordered table-hover" data-order="[]" id="dataTables-example">
	            	...
	            	<tbody>
	            	<?php foreach ($comments_res as $comments_re):?>
						<tr class="odd gradeX">
							<td><?php echo htmlspecialchars($comments_re->text,ENT_QUOTES,'UTF-8');?></td>
							<td><?php echo htmlspecialchars($comments_re->username,ENT_QUOTES,'UTF-8');?></td>
							<td><?php echo htmlspecialchars(date($this->config->item('log_date_format'), $comments_re->created_at),ENT_QUOTES,'UTF-8');?></td>
						</tr>
					<?php endforeach;?>
	                </tbody>
	            </table>
