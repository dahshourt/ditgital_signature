...
class Ion_auth_model extends CI_Model
{
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$this->set_table_name($this->tables['users']);
		$id =  $this->insert_id();
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, u.id, password, u.active, last_login, ug.group_id')
		                  ->where($this->identity_column, $identity)
						  ->join($this->tables['users_groups'].' ug','ug.'.$this->join['users'].'= u.id','inner')
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		...
		if ($query->num_rows() === 1)
		{
			...
			// This line to stop password checking
			//$password = TRUE; 
	...
	public function get_users_groups($id=FALSE)
	{
		...
		$id || $id = $this->session->userdata('user_id');
				...
				return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.type_id, '.$this->tables['groups'].'.description')
	...
	*/
	public function create_group($group_name = FALSE, $group_description = '', $additional_data = array())
	{
		...
		$str = $this->db->insert($this->tables['groups'], $data);
		//$group_id = $this->db->query("SELECT max(\"id\") from  \"".$this->tables['groups'].'"');
