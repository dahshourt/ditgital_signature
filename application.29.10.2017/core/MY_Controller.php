...
class MY_Controller extends MX_Controller
{
	...
	public function __construct()
	{
		...
		if ( ! $this->session->group_id && $this->router->method != 'login')
		{
			$this->load->model('auth/Ion_auth_model');
			$this->load->model('permissions/permission_model');
			$group_data = $this->Ion_auth_model->get_users_groups()->result_array();
			foreach ($group_data as $key => $value) {
				$this->group_id['id'] = $value['id'];
				$this->group_id['name'] = $value['name'];
				$this->group_id['type_id'] = $value['type_id'];
				$this->group_id['description'] = $value['description'];
			}
			$res_permissions = $this->permission_model->get_group_permissions($this->group_id['id']);
			...
			$this->session->set_userdata('groups', $this->group_id);
		...
		switch (ENVIRONMENT)
		{
			case 'development':
				...
				foreach ($bechmark_users as $key => $bechmark_user) {
					if (($this->router->method != 'login') && in_array($_SESSION['user_id'], $bechmark_user))
					{
