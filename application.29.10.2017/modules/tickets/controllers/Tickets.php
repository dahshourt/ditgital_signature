...
class Tickets extends MY_Controller {
	...
	public function create()
    {
    	...
    	if ($this->input->post('submit2'))
		{
            ...
            {
                $this->_add_ticket($get_access_fields);
                $this->session->set_flashdata('message', 'The ticket has been created successfully, You can create a new one.');
    ...
    public function _add_ticket($custom_fields)
    {
        ...
        {
            if ($this->input->post($values['name']) OR ! empty ($_FILES['upload']['name']))
            {
		...
		{
	        $last_id = $this->ticket_model->add_items($data_cst_ticket);
	        if ($upload_flag == TRUE)
	        {
	            $this->upload_files($created_at, $last_id);
	        }
	        
			if ( ! empty($data_cst_textcomment))
			{
		        $data_cst_textcomment['ticket_id']= $last_id;
		...
		foreach ($input_fun['get_access_fields'] as $value_access_field)
		{
			if ($value_access_field['type'] == 'dropdown')
			{
				switch ($value_access_field['name']) {
                    case 'group_id':
                        $data[$value_access_field['name']] = $groups_groups;
                        break;
	...
	{
    	if ($ticket_id == '' OR empty($this->ticket_model->check_ticket_access($ticket_id)))
        {
