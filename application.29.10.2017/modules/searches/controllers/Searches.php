...
class Searches extends MY_Controller
{
	...
	public function index()
	{
		...
		if ($this->form_validation->run() OR ($this->input->post() && $validation != TRUE))
		{
			$data_search = array();
		...
		{
			if ($value['group_access'] == $this->input->post('group_id') && $value['rule_slug'] == 'view_ticket')
			{
				$data['view_ticket'] = TRUE;
			}
			else if ($value['group_access'] == $this->input->post('group_id') && $value['rule_slug'] == 'edit_ticket')
			{
